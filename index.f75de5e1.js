...
buttonRight.addEventListener("click", ()=>{
    ...
    if (count == numberOfSlides - 2) buttonRight.classList.add("slider-button--disabled");
    buttonLeft.classList.remove("slider-button--disabled");
    sliderList.style.transform = `translateX(-${(count + 1) * 100}%)`;
    count++;
});
if (count === 0) buttonLeft.classList.add("slider-button--disabled");
buttonLeft.addEventListener("click", ()=>{
    if (count === 0) {
        buttonLeft.classList.add("slider-button--disabled");
        return;
    }
    if (count === 1) buttonLeft.classList.add("slider-button--disabled");
